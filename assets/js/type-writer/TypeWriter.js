        ...
        if(!document.querySelector(`#typewriter-effect-${this.id}`)){
          ...
          if(typeof this.options.cursor === 'object'){
            if(this.options.cursor['enabled'] !== false){
              ...
              this.element.innerHTML += `<span id="typewriter-effect-${this.id}" class="inline-block transition-transform duration-300 delay-700 before:content-[''] before:bg-primary before:absolute before:top-1/2 before:-translate-y-1/2 before:left-[0.06em] before:w-[200%] before:h-full before:rounded-l-full before:transition-all before:duration-300" style="font-family:'Soehne Circle';"><span class="relative">●</span></span>`;
            }
          } else {
              // this.element.innerHTML += `<span id="typewriter-effect-${this.id}">|</span>`;
              this.element.innerHTML += `<span id="typewriter-effect-${this.id}" class="inline-block transition-transform duration-300 delay-700 before:content-[''] before:bg-primary before:absolute before:top-1/2 before:-translate-y-1/2 before:left-[0.06em] before:w-[200%] before:h-full before:rounded-l-full before:transition-all before:duration-300" style="font-family:'Soehne Circle';"><span class="relative">●</span></span>`;
